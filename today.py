...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    ...
    root = tree.getroot()
    fmt = lambda n: '{:,}'.format(n) if isinstance(n, int) else str(n)
    # right edge is a fixed 61 columns; lengths below are what's left after the static text on each row
    justify_format(root, 'age_data', age_data, 50)
    justify_format(root, 'commit_data', commit_data, 22)
    justify_format(root, 'star_data', star_data, 18 - len(fmt(contrib_data)))
    justify_format(root, 'repo_data', repo_data, 6)
    justify_format(root, 'contrib_data', contrib_data)
    justify_format(root, 'follower_data', follower_data, 12)
    justify_format(root, 'loc_data', loc_data[2], 16 - len(fmt(loc_data[0])))
...
if __name__ == '__main__':
    """
    # slenderongithub, 2026
    """
    print('Calculation times:')
    # define global variable for owner ID and calculate user's creation date
    # e.g {'id': '...'} for username 'slenderongithub'
    ...
    # archived data logic removed (specific to Andrew6rant)
